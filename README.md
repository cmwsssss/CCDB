...
#### Easy-to-use:
CCDB is very easy to use, Just one line of code to insert, query, delete and update, The programmer does not need to be concerned with any database level operations, such as transactions, database connection pooling, thread safety, etc, CCDB will optimize the API operations at the application level to ensure efficient operation at the database level
